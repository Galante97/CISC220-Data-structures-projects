...
void bookLL::printLL() {
	...
	while (tmp != NULL) {
		...
		cout << i << ".) Title:" << tmp->bookTitle << "   Author:" << tmp->author
...
void bookLL::categoryView() {
	...
	if (!cin) {
		...
		string choosenCat = catArr[input].cat;

		bNode *tmp = first;
		while (tmp != NULL) {
			if (tmp->category == choosenCat) {
				cout << "Title: " << tmp->bookTitle << " Author: "
						<< tmp->author << " Category: " << tmp->category
						<< " Year Published: " << tmp->yearPublished
						<< " Pages: " << tmp->pages << endl;
			}
			tmp = tmp->next;
		}
